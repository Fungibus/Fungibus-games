...
export async function handleJustOneApiRequest(request, env, url) {
  if (url.pathname === "/api/just-one/rooms" || url.pathname === "/api/just-one/rooms/") {
    if (request.method !== "POST") return methodNotAllowed();
    return createApiRoom(request, env);
  }

  const joinMatch = url.pathname.match(/^\/api\/just-one\/rooms\/([^/]+)\/join\/?$/);
  if (joinMatch) {
    if (request.method !== "POST") return methodNotAllowed();
    return joinApiRoom(request, env, joinMatch[1]);
  }

  const socketMatch = url.pathname.match(/^\/api\/just-one\/rooms\/([^/]+)\/socket\/?$/);
  if (socketMatch) {
...
function getRoomStub(env, roomCode) {
  const id = env.JUST_ONE_ROOMS.idFromName(roomCode);
  return env.JUST_ONE_ROOMS.get(id);
}

function roomShareUrl(baseUrl, roomCode) {
  const url = new URL("/just-one/", baseUrl);
