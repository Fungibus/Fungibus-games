...
export async function handleMagicalAthleteApiRequest(request, env, url) {
  if (url.pathname === "/api/magical-athlete/rooms" || url.pathname === "/api/magical-athlete/rooms/") {
    if (request.method !== "POST") return methodNotAllowed();
    return createApiRoom(request, env);
  }

  const joinMatch = url.pathname.match(/^\/api\/magical-athlete\/rooms\/([^/]+)\/join\/?$/);
  if (joinMatch) {
    if (request.method !== "POST") return methodNotAllowed();
    return joinApiRoom(request, env, joinMatch[1]);
  }

  const socketMatch = url.pathname.match(/^\/api\/magical-athlete\/rooms\/([^/]+)\/socket\/?$/);
  if (socketMatch) {
...
async function createApiRoom(request, env) {
  ...
  for (let attempt = 0; attempt < MAX_ROOM_CREATE_ATTEMPTS; attempt += 1) {
    ...
    if (response.status !== 409) {
      ...
      return json({ ...payload, shareUrl: roomShareUrl(request.url, payload.roomCode || roomCode) });
    }
  }
...
function getRoomStub(env, roomCode) {
  const id = env.MAGICAL_ATHLETE_ROOMS.idFromName(roomCode);
  return env.MAGICAL_ATHLETE_ROOMS.get(id);
}

function roomShareUrl(baseUrl, roomCode) {
  const url = new URL("/magical-athlete/", baseUrl);
