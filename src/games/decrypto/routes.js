...
export async function handleDecryptoApiRequest(request, env, url) {
  if (url.pathname === "/api/decrypto/rooms" || url.pathname === "/api/decrypto/rooms/") {
    if (request.method !== "POST") return methodNotAllowed();
    return createApiRoom(request, env);
  }

  const joinMatch = url.pathname.match(/^\/api\/decrypto\/rooms\/([^/]+)\/join\/?$/);
  if (joinMatch) {
    if (request.method !== "POST") return methodNotAllowed();
    return joinApiRoom(request, env, joinMatch[1]);
  }

  const socketMatch = url.pathname.match(/^\/api\/decrypto\/rooms\/([^/]+)\/socket\/?$/);
  if (socketMatch) {
...
function getRoomStub(env, roomCode) {
  const id = env.DECRYPTO_ROOMS.idFromName(roomCode);
  return env.DECRYPTO_ROOMS.get(id);
}

function roomShareUrl(baseUrl, roomCode) {
  const url = new URL("/decrypto/", baseUrl);
