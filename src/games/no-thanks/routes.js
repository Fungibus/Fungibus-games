...
export async function handleNoThanksApiRequest(request, env, url) {
  if (url.pathname === "/api/no-thanks/rooms" || url.pathname === "/api/no-thanks/rooms/") {
    if (request.method !== "POST") return methodNotAllowed();
    return createApiRoom(request, env);
  }

  const joinMatch = url.pathname.match(/^\/api\/no-thanks\/rooms\/([^/]+)\/join\/?$/);
  if (joinMatch) {
    if (request.method !== "POST") return methodNotAllowed();
    return joinApiRoom(request, env, joinMatch[1]);
  }

  const socketMatch = url.pathname.match(/^\/api\/no-thanks\/rooms\/([^/]+)\/socket\/?$/);
  if (socketMatch) {
...
function getRoomStub(env, roomCode) {
  const id = env.NO_THANKS_ROOMS.idFromName(roomCode);
  return env.NO_THANKS_ROOMS.get(id);
}

function roomShareUrl(baseUrl, roomCode) {
  const url = new URL("/no-thanks/", baseUrl);
